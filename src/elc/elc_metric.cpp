...
double
elc_metric::avg_contention_time(encoding_sptr enc, uint8_t txc) const
{
   return max_contention_time(enc, txc) / 2.0;
}

double
elc_metric::max_contention_time(encoding_sptr enc, uint8_t txc) const
{
...
double
elc_metric::rts_cts_time(encoding_sptr enc, uint32_t frame_sz, bool short_preamble) const
{
   double usecs = 0.0;
